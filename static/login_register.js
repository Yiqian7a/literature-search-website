...
RegisterEmail.oninput = () => {LoginEmail.value = RegisterEmail.value}
LoginEmail.oninput = () => {RegisterEmail.value = LoginEmail.value}
...
// 同步左右表单的密码
Lpassword.oninput = () => {Rpassword.value = Lpassword.value}
// 验证两次密码输入是否相同
Rpassword.oninput=function(){
	...
	} else { Rcpassword.style.color = ''} // 如果两次输入的密码一致，恢复默认颜色
};
Rcpassword.oninput = function(){
	if (Rpassword.value !== Rcpassword.value){
		Rcpassword.style.color = 'red';
	} else { Rcpassword.style.color = ''}
};
